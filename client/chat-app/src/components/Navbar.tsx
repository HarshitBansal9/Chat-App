  ...
  return (
    <div className="bg-navbar_background h-[100dvh] w-1/6 flex flex-col p-2">
        ...
        <div className='flex flex-col'>
            {navbarLinksTop.filter(link=>link.auth&&session!=null).map((link) =>{return (<Navbar_option selected={(path===('/'+link.name.toLowerCase()))?(true):false} key={link.to} to={link.to} icon={link.icon} children={<div className='text-xl text-gray-400 font-varela'>{link.name}</div>}></Navbar_option>)} )}
        </div>
        <div className='flex flex-col pt-10 mt-auto mb-4'>
            {navbarLinksBottom.map((link) =>{return (<Navbar_option selected={(path===('/'+link.name.toLowerCase()))?(true):false} key={link.to} to={link.to} icon={link.icon} children={<div className='text-xl text-gray-400 font-varela'>{link.name}</div>}></Navbar_option>)} )}
        </div>
    </div>
    ...
    return (
        <div onClick={()=>{
            ...
            navigate(props.to)}} className='z-0 flex flex-row items-center relative'>
                {props.selected?(<div className='absolute top-0 left-0 z-10 bg-gradient-to-b from-violet-900 to-gray-200 w-1 h-full'></div>):(<div className='z-10'></div>)}
