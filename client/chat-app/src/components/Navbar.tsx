  ...
  return (
    <div className="bg-navbar_background h-[100vh] flex flex-col">
        ...
        <div className='flex flex-col'>
            {navbarLinksTop.map((link) =>{return (<Navbar_option selected={(path===('/'+link.name.toLowerCase()))?(true):false} key={link.to} to={link.to} icon={link.icon} children={<div className='text-2xl text-gray-400 font-pixel'>{link.name}</div>}></Navbar_option>)} )}
        </div>
...
function Navbar_option(props: BoxProps) {
    ...
    return (
        <div onClick={()=>navigate(props.to)} className='z-0 flex flex-row items-center'>
            <div className="flex relative items-center gap-4 brightness-110 hover:cursor-pointer hover:brightness-150 pl-8 pr-10 pt-4 pb-4">
                {props.selected?(<div className='absolute top-0 left-0 z-10 bg-gradient-to-r from-violet-950 to-navbar_background w-6 h-full'></div>):(<div className='z-10 w-1'></div>)}
                {props.icon}
                {props.children}
            </div>
        </div>
